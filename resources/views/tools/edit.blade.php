...
@section('page-title', 'Modifier l\'outil: ' . $tool->name)
        ...
        <form action="{{ route('tools.update', $tool) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1 md:col-span-2 space-y-4">
                    ...
                    <select name="api_connection_id" required class="w-full bg-slate-900/50 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-inner">
                        ...
                        @foreach($connections as $c)
                            <option value="{{ $c->id }}" {{ old('api_connection_id', $tool->api_connection_id) == $c->id ? 'selected' : '' }}>
                                {{ $c->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-4">
                    ...
                    <input type="text" name="name" value="{{ old('name', $tool->name) }}" required class="w-full bg-slate-900/50 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                ...
                <div class="space-y-4">
                    ...
                    <input type="text" name="label" value="{{ old('label', $tool->label) }}" required class="w-full bg-slate-900/50 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                ...
                <div class="col-span-1 md:col-span-2 space-y-4">
                    ...
                    <div class="flex">
                        ...
                        <input type="text" name="endpoint" value="{{ old('endpoint', $tool->endpoint) }}" placeholder="/api/endpoint" required class="flex-1 min-w-0 block w-full px-4 py-3 rounded-none rounded-r-xl bg-slate-900/50 border border-slate-700 text-slate-200 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                    </div>
                ...
                <div class="col-span-1 md:col-span-2 space-y-4">
                    ...
                    <input type="text" name="keywords" value="{{ old('keywords', is_array($tool->keywords) ? implode(', ', $tool->keywords) : $tool->keywords) }}" placeholder="ventes, ca, aujourd'hui" required class="w-full bg-slate-900/50 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                ...
                <div class="col-span-1 md:col-span-2 space-y-4">
                    ...
                    <textarea name="description" rows="2" placeholder="Que fait cet outil ?" class="w-full bg-slate-900/50 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">{{ old('description', $tool->description) }}</textarea>
                </div>

                {{-- Active --}}
                <div class="col-span-1 md:col-span-2 flex items-center gap-3 pt-4 border-t border-slate-800/50 mt-4">
                    <input type="checkbox" name="active" id="active" value="1" {{ old('active', $tool->active) ? 'checked' : '' }} class="w-5 h-5 rounded border-slate-600 bg-slate-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900">
